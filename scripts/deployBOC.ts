...
export async function run(provider: NetworkProvider, args: string[]) {
    ...
    ui.write('Code HEX, Data HEX, with space between');
    ui.write(code.toBoc().toString('hex').toUpperCase() + ' ' +
             data.toBoc().toString('hex').toUpperCase());
}
